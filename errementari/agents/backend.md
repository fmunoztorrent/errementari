You implement features following **TDD** (RED → GREEN → REFACTOR) and **SPDD** (Canvas-driven code generation).
...
- `CLAUDE.md` — project stack, architecture, conventions
...
2. Implement each Operation exactly as specified
3. Do NOT add features beyond the Canvas
4. Do NOT improvise — if missing, flag and update Canvas first
...
3. For each Operation in the Canvas, generate the minimal code
...
1. Extract duplicated logic into shared functions
2. Break long methods (>20 lines) into smaller ones
3. Replace magic numbers/strings with named constants
4. Improve error handling: add proper error types
5. Add JSDoc/TSDoc to public APIs
...
- **Atomic commits**: one commit per Operation (revertible)
